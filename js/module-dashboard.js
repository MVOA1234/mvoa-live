...
// dropdown at the top — every tile re-reads against whichever range
// is selected, not just "today". Definitions used throughout:
//   - "Period" tickets/tasks = CreatedDate falls inside the selected
//     range (same shape as the rest of the app's weekly/monthly
//     reports).
...
const DashboardModule = (function () {
  ...
  function computeDailyOpsStats(tasks, range, assigneeOptions) {
    ...
    const openInPeriod = createdInPeriod.filter(t => t.Status === 'Open');
    const openDailyOps = openInPeriod.filter(t => !isPlantRoundsTask(t));
    const openFailedTasks = openInPeriod.filter(isPlantRoundsTask);

    // By Assignee — top 3 by open-count, scoped to this same period's
    // open tickets (so it moves with the dropdown like everything else
    // here). This is a different scope than Daily Operations' own
    // Reports → By Assignee screen, which always shows ALL-time open
    // workload regardless of a date filter — don't expect the two to
    // match number-for-number.
    const byAssignee = {};
    openInPeriod.forEach(t => {
